...
| RF-011 | Sprint 7 | Build/Test | Medium | Open | `frontend/src/lib/auth.ts` | Vite TypeScript validation fails because exported Appwrite helper functions infer non-nameable return types from the Appwrite package | Frontend production builds cannot complete even though runtime authentication code is unaffected | Add explicit public return types using stable Appwrite model types or a narrow application-facing type | Appwrite helper type review | Discovered during Phase 7 validation |
