...
| RF-011 | Sprint 13 | Reliability | Medium | Open | `frontend/src/pages/OutboundRequests.tsx`, `frontend/src/pages/MidmileRequests.tsx`, `frontend/src/pages/DockingConfirmation.tsx` | Frontend request write routes now generate and reuse stable `Idempotency-Key` headers per logical operation, with pending guards on direct mutation buttons | Retries are replay-safe and obvious double submissions are blocked | Keep the shared helper aligned with `backend/app/Http/Middleware/IdempotencyMiddleware.php`; this delivers the medium-term idempotency-key recommendation in `docs/scalability-review.md` | Backend idempotency middleware | Notification read receipts already have a frontend consumer in `frontend/src/components/AppHeader.tsx`; no further frontend work is needed for that feature |
