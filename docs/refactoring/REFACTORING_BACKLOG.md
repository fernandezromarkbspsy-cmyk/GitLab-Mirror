...
| RF-011 | Phase 9 final review | Testing | High | Open | `frontend/src/components/AppHeader.tsx`, `frontend/src/pages/Overview.tsx`, `frontend/src/pages/MidmileRequests.tsx`, `frontend/src/pages/UserManagement.tsx` | Code graph identifies these high-degree dashboard hubs as lacking direct focused coverage | UI regressions in navigation, polling, role switching, and operational summaries may reach production undetected | Add focused component or interaction tests for notification polling, role navigation, dashboard query states, and permission-sensitive views | Frontend test harness and stable API mocks | Identified by final Code Review Graph review; `OutboundRequests.tsx` is already tracked by RF-007 |
