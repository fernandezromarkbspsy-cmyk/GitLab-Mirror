...
function Reveal({ children, className = "", delay = 0 }) {
    ...
    useEffect(() => {
        ...
        if (!node)
            return;
        const o = new IntersectionObserver(([e]) => { if (e.isIntersecting) {
...
function Sidebar({ isOpen, close, active, setActive, pushToast }) {
...
function KpiCard({ icon, label, value, delta, deltaTone, note, children, dark = false, delay = 0, onClick, active }) {
    return (<Reveal delay={delay} className="kpi-reveal">
      <article className={`kpi-card ${dark ? "dark" : ""} ${active ? "kpi-active" : ""} ${onClick ? "clickable" : ""}`} onClick={onClick} role={onClick ? "button" : undefined} tabIndex={onClick ? 0 : undefined} onKeyDown={e => { if (onClick && (e.key === "Enter" || e.key === " "))
        onClick(); }}>
...
function Gauge({ value, tone = "lime", className = "" }) {
...
function MonthlySelect({ value, onChange }) {
...
function RequestChart({ period, setPeriod, activeCat, setActiveCat, pushToast }) {
    ...
    const xLabels = useMemo(() => {
        if (period === "Monthly")
            return ["Electronic", "Drinks", "Watch", "Fashion", "Fashion", "Toys"];
        if (period === "Weekly")
    ...
    return (<section className="panel chart-panel" id="lh-request">
      ...
      <div className="chart-x-labels">
        {xLabels.map((lbl, i) => <button key={lbl + i} className={activeCat === lbl || (period === "Monthly" && ["Fashion"].includes(lbl) && activeCat === "Fashion" && i === 3) ? "active" : ""} onClick={() => { if (period === "Monthly")
        setActiveCat(lbl);
    else
        pushToast(`${lbl}: ${data.request[i % data.request.length] * 1000} requests`, "info"); }}>{lbl}</button>)}
...
export default function App() {
    ...
    const [docking, setDocking] = useState(initialDocking);
    ...
    const [dockingStatusFilter, setDockingStatusFilter] = useState("All");
    const [sortInv, setSortInv] = useState(null);
    const [sortDock, setSortDock] = useState(null);
    ...
    const filteredInventory = useMemo(() => {
        ...
        if (activeTag && activeTag !== "All")
            arr = arr.filter(i => i.tag === activeTag);
        if (globalSearch) {
        ...
        if (sortInv) {
            arr.sort((a, b) => {
                const av = a[sortInv.key];
                const bv = b[sortInv.key];
                if (typeof av === "number" && typeof bv === "number")
                    return (av - bv) * sortInv.dir;
    ...
    const filteredDocking = useMemo(() => {
        let arr = [...docking];
        if (dockingStatusFilter !== "All")
            arr = arr.filter(d => d.status === dockingStatusFilter);
        const q = (globalSearch || dockingSearch).toLowerCase();
        if (q)
            arr = arr.filter(d => d.name.toLowerCase().includes(q) || d.lhId.toLowerCase().includes(q) || d.detail.toLowerCase().includes(q) || d.status.toLowerCase().includes(q));
        if (sortDock) {
            arr.sort((a, b) => {
                const av = a[sortDock.key];
                const bv = b[sortDock.key];
                if (typeof av === "number" && typeof bv === "number")
                    return (av - bv) * sortDock.dir;
    ...
    useEffect(() => { const h = () => { if (window.innerWidth > 820)
        setSidebarOpen(false); }; window.addEventListener("resize", h); return () => window.removeEventListener("resize", h); }, []);
    const handleAddProduct = (data) => {
        const newItem = { ...data, id: Math.random().toString(36).slice(2, 7) };
        setInventory(v => [newItem, ...v]);
        if (data.request) {
            const newDock = { id: Math.random().toString(36).slice(2, 7), lhId: `LR-${2049 + inventory.length + 1}`, name: data.name, detail: `${data.stock} In Stock`, request: data.request, status: "Pending", tone: "yellow", imageKey: data.imageKey };
    ...
    const handleDeleteDock = (id) => {
        setDocking(v => v.filter(x => x.id !== id));
        push("Request removed", "info");
    };
    const toggleDockStatus = (id) => {
        setDocking(v => v.map(d => d.id === id ? { ...d, status: d.status === "Assigned" ? "Pending" : "Assigned", tone: d.status === "Assigned" ? "yellow" : "green" } : d));
        push("Status updated", "success");
    };
    const handleAddTag = () => {
        if (!newTagName.trim())
            return push("Tag name required", "error");
        if (tags.includes(newTagName.trim()))
            return push("Tag already exists", "error");
    ...
    const currentTruck = truckDatasets[truckPeriod];
    return (<div className="app-shell">
      ...
      <div className="main-shell">
        <Header onMenu={() => setSidebarOpen(true)} search={globalSearch} setSearch={setGlobalSearch} onAdd={() => setShowAdd(true)} notifications={notifData} messages={msgData} pushToast={push} onFilterChange={(k) => { if (k === "all")
        setDockingStatusFilter("All");
    else {
        setChartPeriod("Monthly");
        push(`Filter applied: ${k}`);
    } }}/>
...
function ProductForm({ initial, onSubmit, onCancel, tags }) {
    ...
    return (<form className="form-grid" onSubmit={e => { e.preventDefault(); if (invalid)
        return; const priceNum = parseInt(price.replace(/\D/g, "")) || 0; onSubmit({ name: name.trim(), sku, qty: parseInt(qty) || 0, stock: parseInt(stock) || 0, price, priceNum, tag, imageKey, request: parseInt(request) || 0 }); }}>
...
function DockForm({ initial, onSubmit, onCancel }) {
    const [name, setName] = useState(initial.name);
    const [detail, setDetail] = useState(initial.detail);
    const [request, setRequest] = useState(String(initial.request));
    const [status, setStatus] = useState(initial.status);
    return (<form className="form-grid" onSubmit={e => { e.preventDefault(); onSubmit({ name, detail, request: parseInt(request) || 0, status, tone: status === "Assigned" ? "green" : status === "Pending" && detail.toLowerCase().includes("low") ? "red" : "yellow" }); }}>
      ...
      <div className="form-row-2"><label>Request units<input type="number" value={request} onChange={e => setRequest(e.target.value)}/></label><label>Status<select value={status} onChange={e => setStatus(e.target.value)}><option>Assigned</option><option>Pending</option></select></label></div>
