...
export function AppHeader({ user, view, onRoleChange, onSearch }: Props) {
  ...
  useEffect(() => {
    ...
    function handlePointerDown(event: MouseEvent) {
      ...
      if (filterOpen && filterMenuRef.current && !filterMenuRef.current.contains(target)) setFilterOpen(false);
      if (mailOpen && mailMenuRef.current && !mailMenuRef.current.contains(target)) setMailOpen(false);
  ...
  return <header className="app-topbar">
    ...
    <div className="topbar-page">
      <div className="topbar-page-copy">
        ...
        <nav className="topbar-breadcrumbs" aria-label="Breadcrumb"><span className="breadcrumb-root">Operations</span><ChevronRight size={12} aria-hidden="true"/><span className="breadcrumb-current" aria-current="page">{page[view].section}</span></nav>
      </div>
    </div>
    <div className="topbar-tools">
      ...
      <div ref={filterMenuRef} className="topbar-menu-wrap">
        <button className={`top-icon-button ${filterOpen ? 'selected' : ''}`} type="button" onClick={() => setFilterOpen(value => !value)} aria-label="Filter dashboard" aria-expanded={filterOpen}><SlidersHorizontal size={19} /></button>
        {filterOpen && <section className="topbar-popover filter-popover" role="menu" aria-label="Dashboard filters">
          <strong>Quick filters</strong>
          <button type="button" role="menuitem" onClick={() => { const end = new Date(); const start = new Date(end); start.setDate(start.getDate() - 29); setDateRange(formatDate(start), formatDate(end)); setFilterOpen(false); }}>Last 30 days <span>✓</span></button>
          <button type="button" role="menuitem" onClick={() => { resetDateRange(); setFilterOpen(false); }}>All requests <span>✓</span></button>
          <button type="button" role="menuitem" onClick={() => { setSearch(''); resetDateRange(); setFilterOpen(false); }}>Clear filters <X size={13} /></button>
        </section>}
      </div>
      <div className="notification-menu">
        <button ref={notificationButtonRef} className="top-icon-button notification-button" type="button" title="Notifications" aria-label={`Open notifications, ${count} unread`} aria-expanded={open} aria-controls={notificationMenuId} onClick={()=>{setOpen(value=>!value);setMailOpen(false);setFilterOpen(false);}}><Bell size={19}/>{count>0&&<span>{count>99?'99+':count}</span>}</button>
      ...
      <div ref={mailMenuRef} className="topbar-menu-wrap">
        <button className="top-icon-button mail-button" type="button" onClick={() => { setMailOpen(value => !value); setOpen(false); setFilterOpen(false); }} aria-label="Messages" aria-expanded={mailOpen}><Mail size={19} />{count > 0 && <i className="badge-dot mail-dot" />}</button>
        {mailOpen && <section className="notification-popover mail-popover" role="menu" aria-label="Messages">
          <div><strong>Messages</strong><span className="popover-count">{count} new</span></div>
          {alerts.length ? <div className="notification-list">{alerts.slice(0, 6).map(item => <button key={item.id} className={item.read_at ? '' : 'unread'} type="button" role="menuitem" onClick={() => { if (!item.read_at) read.mutate(item.id); setMailOpen(false); }}><Mail size={14} /><span>{item.title}</span><small>{item.body}</small></button>)}</div> : <p>No new messages.</p>}
