...
export function LinehaulRequestDetailsPanel({
  request,
  onClose,
  onNotice,
  position,
  ...
  useEffect(() => {
    ...
    const closeOnBackgroundClick = (event: PointerEvent) => {
  ...
  useEffect(() => {
    const movePanel = (event: PointerEvent) => {
      ...
      const x =
        (event.clientX - workspaceBounds.left) / scale -
        dragRef.current.offsetX / scale;
      const y =
        (event.clientY - workspaceBounds.top) / scale -
        dragRef.current.offsetY / scale;
      onPositionChange({
        x: Math.max(
          8,
          Math.min(
            x,
            workspaceBounds.width / scale - panelBounds.width / scale - 8,
          ),
        ),
        y: Math.max(
          8,
          Math.min(
            y,
            workspaceBounds.height / scale - panelBounds.height / scale - 8,
          ),
        ),
      });
    };
  ...
  const tabs = [
    { id: "details" as const, label: "Request details" },
    ...
    >
      <header className="lh-details-panel-header" onPointerDown={startDragging}>
        ...
        <div className="lh-details-panel-actions">
          <button
            type="button"
            aria-label="Open request in new view"
            onClick={() => onNotice(`Opening ${request.id}`)}
          >
      ...
      <div className="lh-details-panel-summary">
        <div className="lh-details-panel-avatar" aria-hidden="true">
      ...
      >
        {tabs.map((tab) => (
          ...
          >
            {tab.id === "details" && <CheckCircle2 size={14} />}
            {tab.label}
          </button>
        ))}
      </div>
      {activeTab === "details" && (
        <div id="lh-panel-details" role="tabpanel">
          <dl className="lh-details-panel-fields">
            ...
            <div>
              ...
              <dd>{valueOf(request.truck_size)}</dd>
            </div>
            <div>
              <dt>
                <UserRound size={14} />
            ...
            <div>
              ...
              <dd>{valueOf(request.linehaul_trip_no)}</dd>
            </div>
            <div>
              <dt>
                <Truck size={14} />
              ...
              <dd>{valueOf(request.plate_number)}</dd>
            </div>
          </dl>
        </div>
      )}
      {activeTab === "docs" && (
        <div
          id="lh-panel-docs"
          className="lh-details-panel-tab-content"
          role="tabpanel"
        >
          <ClipboardCopy size={18} />
          <strong>Request documents</strong>
          <span>No documents are attached to this request.</span>
        </div>
      )}
      <footer className="lh-details-panel-footer">
        <button
          type="button"
          onClick={() => onNotice(`Exporting ${request.id}`)}
        >
          <ArrowUp size={15} />
          Export
        </button>
