...
import { BadgeCheck, ChartNoAxesCombined, ChevronLeft, ChevronRight, CircleCheck, Clock3, Hash, ListChecks, MoreHorizontal, RefreshCw, Save, Search, SlidersHorizontal, Table2, Tag, Truck, Users, X } from 'lucide-react';
...
import type { ClusterLookup, Page, RequestFilters, RequestSort, TruckRequest, User } from '../types';
...
export function OutboundRequests({ user, queue: _queue }: { user: User; queue: QueueSnapshot }) {
  ...
  async function refreshData(message: string) { showToast(message); await queryClient.invalidateQueries({ queryKey: ['requests'] }); }
  ...
  function selectRow(row: TruckRequest, event: MouseEvent<HTMLElement>) {
    const workspace = event.currentTarget.closest<HTMLElement>('.lh-request-workspace');
    const bounds = workspace?.getBoundingClientRect();
    const scale = window.matchMedia('(min-width: 821px)').matches ? 0.75 : 1;
    if (bounds) setPanelPosition({ x: Math.max(8, Math.min((event.clientX - bounds.left + 12) / scale, bounds.width / scale - 328)), y: Math.max(8, Math.min((event.clientY - bounds.top + 12) / scale, bounds.height / scale - 360)) });
    setSelectedRow(row);
  }
  ...
  return <div className="workspace-view lh-request-page" aria-label="Linehaul request workspace"><section className="lh-request-workspace" aria-label="Linehaul requests">
    <LinehaulFilterPanel filters={filters} onChange={setFilters} onSort={sortBy} onExport={() => void exportRows()} onAddNew={() => { createRequest.reset(); setCreating(true); }} onNotice={showToast} />
    {creating && <div className="lh-table-create-row"><InlineCreateRow busy={createRequest.isPending} error={createRequest.error?.message} onCancel={() => { createRequest.reset(); setCreating(false); }} onSubmit={payload => createRequest.mutate(payload)} /></div>}
    {selectedRow && <LinehaulRequestDetailsPanel request={selectedRow} position={panelPosition} onPositionChange={setPanelPosition} onClose={() => setSelectedRow(null)} onNotice={showToast} />}
    <section className="lh-table-shell" aria-label="Outbound linehaul request records"><div className="lh-table-toolbar"><div className="lh-view-controls"><button className="lh-toolbar-icon" type="button" aria-label="Refresh records" onClick={() => void requests.refetch()}><RefreshCw size={16} /></button><div className="lh-view-tabs"><button type="button" onClick={() => showToast('Chart view is coming soon')}><ChartNoAxesCombined size={15} />Chart</button><button className={view === 'table' ? 'selected' : ''} type="button" onClick={() => setView('table')}><Table2 size={15} />Table</button><button className={view === 'card' ? 'selected' : ''} type="button" onClick={() => setView('card')}><SlidersHorizontal size={15} />Card</button></div></div><label className="lh-search-box"><Search size={17} /><input value={filters.search} onChange={event => updateSearch(event.target.value)} placeholder="Search by plate number" /><kbd>Ctrl + F</kbd></label></div>
      {view === 'table' ? <div className="lh-records-table" role="table"><div className="lh-table-head lh-table-grid" role="row"><span><CircleCheck size={14} />Status</span><button type="button" onClick={() => sortBy('request_timestamp')}><Clock3 size={14} /><span>Request Time</span><SlidersHorizontal size={13} /></button><button type="button" onClick={() => sortBy('cluster')}><Hash size={14} /><span>Cluster</span><SlidersHorizontal size={13} /></button><span><BadgeCheck size={14} />Region</span><button type="button" onClick={() => sortBy('dock_no')}><Truck size={14} /><span>Dock #</span><SlidersHorizontal size={13} /></button><button type="button" onClick={() => sortBy('backlogs')}><ListChecks size={14} /><span>Backlogs</span><SlidersHorizontal size={13} /></button><span><Truck size={14} />LH Size</span><span><Users size={14} />SOC PIC</span><span><Tag size={14} />LH Trip #</span><button type="button" onClick={() => sortBy('plate_number')}><Hash size={14} /><span>Plate #</span><SlidersHorizontal size={13} /></button><span /></div><div className="lh-table-body">{requests.isPending && <div className="lh-empty-state">Loading live requests...</div>}{requests.error && <div className="lh-empty-state">{requests.error.message}</div>}{!requests.isPending && !requests.error && rows.map((row, index) => <div className="lh-table-row lh-table-grid" role="row" tabIndex={0} key={row.id} style={{ '--row-index': index } as React.CSSProperties} onClick={event => selectRow(row, event)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSelectedRow(row); } }}><span><span className={`lh-status lh-status-${row.status.toLowerCase()}`}>{statusLabel(row.status)}</span></span><span>{formatDateTime(row.request_timestamp)}</span><span title={row.cluster}>{row.cluster}</span><span>{row.region}</span><span>{row.dock_no}</span><span>{row.backlogs.toLocaleString()}</span><span>{row.truck_size}</span><span>{displayValue(row.ob_fte)}</span><span>{displayValue(row.linehaul_trip_no)}</span><span>{displayValue(row.plate_number)}</span><span className="lh-row-menu-wrap"><button className="lh-row-more" type="button" aria-label={`Actions for request ${row.id}`} onClick={event => { event.stopPropagation(); setOpenRow(openRow === row.id ? null : row.id); }}><MoreHorizontal size={17} /></button>{openRow === row.id && <span className="lh-row-menu"><button type="button" onClick={event => { event.stopPropagation(); selectRow(row, event); setOpenRow(null); }}>View</button><button type="button" onClick={() => showToast(`Editing ${row.id}`)}>Edit</button></span>}</span></div>)}{!requests.isPending && !requests.error && rows.length === 0 && <div className="lh-empty-state">No live requests match the current filters.</div>}</div></div> : <div className="lh-card-view">{rows.map(row => <article className="lh-record-card" key={row.id} tabIndex={0} onClick={event => selectRow(row, event)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSelectedRow(row); } }}><div><small>{row.id}</small><h3>{row.cluster}</h3><p>{row.region} ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· Dock {row.dock_no}</p></div><div className="lh-card-right"><strong>{row.truck_size}</strong><span>{row.backlogs.toLocaleString()} backlogs</span><span className={`lh-status lh-status-${row.status.toLowerCase()}`}>{statusLabel(row.status)}</span></div></article>)}</div>}
    ...
    </section></section>{toast && <div className="lh-toast" role="status">{toast}</div>}</div>;
}
...
function InlineCreateRow({ busy, error, onCancel, onSubmit }: { busy: boolean; error?: string; onCancel: () => void; onSubmit: (payload: RequestPayload) => void }) {
  ...
  function pick(cluster: ClusterLookup) { setSelected(cluster); setClusterText(cluster.cluster_name); }
